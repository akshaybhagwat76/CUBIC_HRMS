...
(function ($) {
	...
	$.fn.numpad = function (options) {
		...
		return this.each(function () {
			...
			if ($('#' + id).length == 0) {
				...
				nmpd.grid = table;
				table.append($(options.rowTpl).append($(options.displayCellTpl).addClass('cssNumPadPaddingPx').append(display).append($('<input type="hidden" class="dirty" value="0"></input>'))));
				// Create rows and columns of the the grid with appropriate buttons
				table.append(
					$(options.rowTpl)
						.append($(options.cellTpl).addClass('cssNumPadBtnPaddingLeft').append($(options.buttonNumberTpl).html(7).addClass('btnKeyPad')))
						.append($(options.cellTpl).append($(options.buttonNumberTpl).html(8).addClass('btnKeyPad')))
						.append($(options.cellTpl).addClass('cssNumPadBtnPaddingRight').append($(options.buttonNumberTpl).html(9).addClass('btnKeyPad')))
						.append($(options.cellTpl).addClass('cssNumPadBtnPaddingRight').append($(options.buttonFunctionTpl).html(options.textDelete).addClass('del').click(function () {
							nmpd.setValue(nmpd.getValue().toString().substring(0, nmpd.getValue().toString().length - 1));
						})))
				).append(
					$(options.rowTpl)
						.append($(options.cellTpl).addClass('cssNumPadBtnPaddingLeft').append($(options.buttonNumberTpl).html(4).addClass('btnKeyPad')))
						.append($(options.cellTpl).append($(options.buttonNumberTpl).html(5).addClass('btnKeyPad')))
						.append($(options.cellTpl).append($(options.buttonNumberTpl).html(6).addClass('btnKeyPad')))
						.append($(options.cellTpl).append($(options.buttonFunctionTpl).html(options.textClear).addClass('clear').click(function () {
							nmpd.setValue('');
						})))
				).append(
					$(options.rowTpl)
						.append($(options.cellTpl).addClass('cssNumPadBtnPaddingLeft').append($(options.buttonNumberTpl).html(1).addClass('btnKeyPad')))
						.append($(options.cellTpl).append($(options.buttonNumberTpl).html(2).addClass('btnKeyPad')))
						.append($(options.cellTpl).append($(options.buttonNumberTpl).html(3).addClass('btnKeyPad')))
						.append($(options.cellTpl).append($(options.buttonFunctionTpl).html(options.textCancel).addClass('cancel cssBtnCancel').click(function () {
							nmpd.close(false);
						})))
				).append(
					$(options.rowTpl)
						.append($(options.cellTpl).addClass('cssNumPadBtnPaddingLeft cssNumPadPaddingBtmPx').append($(options.buttonFunctionTpl1).html('&plusmn;').addClass('neg').click(function () {
							nmpd.setValue(nmpd.getValue() * (-1));
						})))
						.append($(options.cellTpl).addClass('cssNumPadPaddingBtmPx').append($(options.buttonNumberTpl).html(0).addClass('btnKeyPad')))
						.append($(options.cellTpl).addClass('cssNumPadPaddingBtmPx').append($(options.buttonFunctionTpl2).html(options.decimalSeparator).addClass('sep').click(function () {
							nmpd.setValue(nmpd.getValue().toString() + ".");
						})))
						.append($(options.cellTpl).addClass('cssNumPadPaddingBtmPx').append($(options.buttonFunctionTpl).html(options.textDone).addClass('done cssBtnDone')))
				);
				...
				$('#' + id + ' .btnKeyPad').bind('click', function () {
					var val;
					if ($('#' + id + ' .dirty').val() == '0') {
						val = $(this).text();
					} else {
	...
	*/
	...
	$.fn.numpad.defaults = {
		...
		buttonFunctionTpl: '<button class="btnKeyFunc"></button>',
		buttonFunctionTpl1: '<button class="btnKeyFunc1"></button>',
		buttonFunctionTpl2: '<button class="btnKeyFuncDot"></button>',
		gridTableClass: '',
